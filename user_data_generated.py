...
def map_values_to_keys(input_values):
    ...
    mapping_rules = {
        ...
        'Username': lambda x: 'Username' if x.isalpha() and len(x) >= 4 and x[0].isupper() else None
,
...
def csv_to_inputs(csv_path): # if we need csv file for input
    ...
    input_values=pd.read_csv('data.csv')
    input_values["Mobile"]=input_values['Mobile'].astype(str) #int mobile to str 
    return input_values
